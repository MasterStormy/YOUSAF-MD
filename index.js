...
// Owner numbers
const ownerNumber = ["923109506687"];
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/yousaf.html')) {
    console.log("✅ yousaf.html found in lib folder");
  } else {
    console.log("⚠️ yousaf.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("YOUSAF-MD~")) {
    console.log("❌ Invalid YOUSAF-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ YOUSAF-MD session restored successfully");
}
...
// Serve HTML page
app.get('/', (req, res) => {
  const htmlPath = path.join(__dirname, 'lib', 'yousaf.html');
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'YOUSAF'}</p>
        </div>
      </body>
      </html>
    `);
...
app.get('/lib/yousaf.html', (req, res) => {
  res.redirect('/');
});
...
async function connectToWA() {
  ...
  console.log(`🤖 YOUSAF-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ YOUSAF-MD Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '923109506687@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
